...
void *SteelBattalion::pollThreaded(void *ptr)
{
	SteelBattalion::SteelBattalionController *ref = (SteelBattalion::SteelBattalionController *)ptr;
	ref->poll();
	return ptr;
}
...
unsigned char*SteelBattalion::SteelBattalionController::getRawLEDData() const
{
	unsigned char*out = new unsigned char[34];
	for(size_t i= 0; i < 34; ++i)
		out[i] = rawLEDData[i];
	return out;
}
...
unsigned char*SteelBattalion::SteelBattalionController::getRawControlData() const
{
	unsigned char*out = new unsigned char[26];
	for(size_t i= 0; i < 26; ++i)
		out[i] = rawControlData[i];
	return out;
}
	...
	if(!(error = libusb_init(&ctx)))
	{
		...
		if(device != NULL)
		{
			...
			if(!libusb_claim_interface(device, 0))
			{
				...
				pthread_create(&loop, NULL, pollThreaded, (void *)this);
			}
			else
			{
...
{
	unsigned char *temp;
	unsigned char* buffer = new unsigned char[26];
	int done = 0;												// make an integer in which to store the amount of data we were able to transfer (bytes)
	while(((!shutdown) || refreshLED))
	{
		...
		{												// Assuming you were able to get the device claimed, we carry on
			...
			buffer = temp;										//
			usleep(1000*pollingInterval);								// sleep for the number of milliseconds specified until the next update time
		}
	}
	resetLEDs();
	if(libusb_kernel_driver_active(device, 0))                                                      // Check to see if the kernel is using the device
...
void SteelBattalion::SteelBattalionController::setPollingInterval(const int &i)						// This method changes the polling interval. It's not used yet
{
	pollingInterval = i;
}

int SteelBattalion::SteelBattalionController::getPollingInterval() const						// This method returns the polling interval. This works, but it isn't very accurate because it doesn't poll much
{
	return pollingInterval;
}

void SteelBattalion::SteelBattalionController::setLEDState(int light, int intensity, bool refresh)
{
...
int SteelBattalion::SteelBattalionController::getLEDState(ControllerLEDEnum light) const				// returns the LED brightness value from the rawLEDData byte array
{
...
int SteelBattalion::SteelBattalionController::getButtonState(ButtonEnum button) const 					// returns the button state from the rawControlData byte array. It must use the button mask list to find where in the
...
void SteelBattalion::SteelBattalionController::resetLEDs()								// Resets the LED values to 0
{
	for(size_t i = 0; i < 34; ++i)									// Sets every value in the LED array to 0
		rawLEDData[i] = 0;									//
	refreshLED = true;										// Then flags the LEDs to be updated
}

int SteelBattalion::SteelBattalionController::getAimingX() const							// This method returns the integer value of the x axis of the aiming stick (the right joystick on the device)
{
	return int(rawControlData[9])-127;									// Stored in byte 9 of the buffer
}

int SteelBattalion::SteelBattalionController::getAimingY() const							// This method returns the integer value of the y axis of the aiming stick (the right joystick on the device)
{
	return int(rawControlData[11])-127;									// Stored in byte 11 of the buffer
}

int SteelBattalion::SteelBattalionController::getRotationLever() const							// This method returns the integer value of the rotation lever (the left joystick on the device)
{
	return ((int(rawControlData[13] < 128))?int(rawControlData[13]):int(rawControlData[13])-255);// Stored in byte 13 of the buffer
}

int SteelBattalion::SteelBattalionController::getSightChangeX() const							// This method returns the integer value of the x axis of the sight change stick (the thumbstick on the device)
{
	return ((int(rawControlData[15] < 128))?int(rawControlData[15]):int(rawControlData[15])-255);// Stored in byte 15 of the buffer
}

int SteelBattalion::SteelBattalionController::getSightChangeY() const							// This method returns the integer value of the y axis of the sight change stick (the thumbstick on the device)
{
	return ((int(rawControlData[17] < 128))?int(rawControlData[17]):int(rawControlData[17])-255);	// Stored in byte 17 of the buffer
}

int SteelBattalion::SteelBattalionController::getLeftPedal() const							// This method returns the integer value of the Left pedal
{
	return int(rawControlData[19]);									// Stored in byte buffer 19
}

int SteelBattalion::SteelBattalionController::getMiddlePedal() const							// This method returns the integer value of the Middle pedal
{
	return int(rawControlData[21]);									// Stored in byte buffer 21
}

int SteelBattalion::SteelBattalionController::getRightPedal() const							// This method returns the integer value of the Right pedal
{
	return int(rawControlData[23]);									// Stored in byte buffer 23
}

int SteelBattalion::SteelBattalionController::getTunerDial() const							// This method returns the integer value of the tuner dial
{
	return int(rawControlData[24]) & 0x0F;								// Stored in byte buffer 24
}

int SteelBattalion::SteelBattalionController::getGearLever() const							// this method returns the integer value of the gear lever
{
	switch(int(rawControlData[25]))									// Stored in byte buffer 25
	{
		case 254:
			return 0;
		case 255:
...
int SteelBattalion::SteelBattalionController::getError() const
{
	return error;
}
...
SteelBattalion::SteelBattalionController::~SteelBattalionController()							// Destructor
{
	...
	{
		shutdown = true;									// Signal the thread to stop
		while(shutdown)										// Wait for the thread to stop
			;
		pthread_exit(NULL);
